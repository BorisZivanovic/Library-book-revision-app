 ...
 * @author Jaca
 */
public class ModelTabeleStarijeOdluke extends AbstractTableModel{
    
    List<OdlukaUsvajanja> lista;
    String kolone[] = {"idOdluke", "datum odluke", "broj za zalbe", "idOsobe", "idRevizije", "idBiblioteke"};
    
    public ModelTabeleStarijeOdluke(){
        
        try {
        lista = new ArrayList<>();
        lista = Kontroler.getInstance().vratiStarijeOdlukeUsvajanja();
        } catch (Exception ex) {
    ...
    public void refreshTabele() throws Exception {
        List<OdlukaUsvajanja> odlukeLista = Kontroler.getInstance().vratiStarijeOdlukeUsvajanja();
