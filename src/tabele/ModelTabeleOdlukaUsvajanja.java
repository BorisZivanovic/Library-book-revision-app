 ...
 * @author Jaca
 */
public class ModelTabeleOdlukaUsvajanja extends AbstractTableModel{
    
     List<OdlukaUsvajanja> lista;
    String kolone[] = {"idOdluke", "datum odluke", "broj za zalbe", "idOsobe", "idRevizije", "idBiblioteke"};
    
    public ModelTabeleOdlukaUsvajanja(){
        
        try {
        lista = new ArrayList<>();
        lista = Kontroler.getInstance().vratiSveOdlukeUsvajanja();
        } catch (Exception ex) {
    ...
    public void refreshTabele() throws Exception {
        List<OdlukaUsvajanja> odlukeLista = Kontroler.getInstance().vratiSveOdlukeUsvajanja();
